...
| IOL `abandonStart()` | unwind is a no-op | **deferred, by the same decision that leaves IOL untouched.** `device_iol::prepare()` calls `posix_setuid()` in the wrapper's own process before the failure points that follow it, so the `releaseTaps()`/`reapTenant()` unwind runs as the tenant and its `sudo` calls fail. The fix is the one `docs/HANDOVER.md` already names — move IOL onto `device::spawnAsTenant()` so the wrapper stays root — and it is gated on a licensed image, because no IOL node has ever started here and nothing would catch a mistake. `iol-dataplane.sh` drives `iol_wrapper` directly and does not cover this. **Note:** the security-review fixes hardened the in-process drop `device_iol::prepare()` already does (supplementary groups now cleared, uid confirmed, every step checked; `tests/Security/IolPrivilegeDropTest.php`) and bound the serial data plane to loopback — neither needs an image — but the `abandonStart()` unwind still runs as the tenant, so this row stays deferred |
