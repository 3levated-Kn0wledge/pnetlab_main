 ...
 */
class device_qemu_wp extends device
{
    ...
    public function command()
    {
        ...
        $cmd = '/opt/unetlab/wrappers/qemu_wrapper -T ' . escapeshellarg($this->getHost()) . ' -D ' . escapeshellarg($this->getSession()) . ' -P ' . escapeshellarg($port) . ' -t ' . escapeshellarg($this->name) . ' -F ' . escapeshellarg($bin) . ' -d ' . (int) $this->delay;
        if ($this->console != 'telnet'  && $this->console_2nd != 'telnet') {
            // Disable telnet (wrapper) console
            $cmd .= ' -x';
        }

        $cmd .= ' -- ' . $flags . ' > ' . escapeshellarg($this->getRunningPath() . '/wrapper.txt') . ' 2>&1 &';
